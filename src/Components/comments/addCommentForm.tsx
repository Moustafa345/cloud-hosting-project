...
const AddCommentForm = ({articleId}: AddCommentFormProps) => {
    ...
    const formSubmitHandler = async (e:React.FormEvent) => {
        e.preventDefault();
        if(text === '') return toast.error("Your comment is required");
    ...
    return (
        ...
        <form onSubmit={formSubmitHandler} className="">
            ...
            />
            <button type="submit" className="bg-green-700 text-white mt-2 p-1 w-min text-xl rounded-lg hover:bg-green-900">
                Comment
            </button>
        </form>
